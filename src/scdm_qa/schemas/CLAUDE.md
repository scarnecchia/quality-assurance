...
## Contracts
- **Exposes**: `get_schema(table_key) -> TableSchema`, `list_table_keys()`, `build_validation(schema, df) -> Validate`, `parse_spec() -> list[TableSchema]`
- **Guarantees**: Lazy-loaded registry caches parsed schemas. All 19 SCDM tables are parsed from `tables_documentation.json`. `build_validation` produces a pointblank `Validate` object with nullability, enum, length, and conditional rules.
